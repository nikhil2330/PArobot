...
def main():
    ...
    try:
        while True:
            ...
            # START edge: reset state cleanly
            ...
            if not tracked_active:
                # draw all detections (thin yellow) when not locked
                for det in people:
                ...
                if best_match is not None:
                    ...
                    if tracked_lost_frames > MAX_TRACK_LOST_FRAMES:
                        tracked_bbox = None  # stop movement
            ...
            if has_lock_and_bbox:
                ...
                # Aim LiDAR cone based on camera horizontal offset
                aim_angle = offset_norm * (CAMERA_FOV_RAD / 2.0)

                if offset_norm < -CENTER_DEADZONE:
                    target_turn_cmd = +TURN_SPEED_TARGET  # turn left
                elif offset_norm > +CENTER_DEADZONE:
                    target_turn_cmd = -TURN_SPEED_TARGET  # turn right
                else:
                    target_turn_cmd = 0.0
            ...
            # --- DIRECT tank mixing (NO smoothing) ---
            if robot_enabled:
                left  = float(np.clip(target_forward_cmd - target_turn_cmd, -100, 100))
                right = float(np.clip(target_forward_cmd + target_turn_cmd, -100, 100))
                tank(left, right)

                # Optional motion string for logs
                if abs(target_forward_cmd) < 1.0 and abs(target_turn_cmd) < 1.0:
                    motion_str = "STOP"
                elif abs(target_forward_cmd) >= 1.0 and abs(target_turn_cmd) < 1.0:
                    motion_str = "FORWARD" if target_forward_cmd > 0 else "BACKWARD"
                elif abs(target_forward_cmd) < 1.0 and abs(target_turn_cmd) >= 1.0:
                    motion_str = "TURN LEFT IN PLACE" if target_turn_cmd > 0 else "TURN RIGHT IN PLACE"
                else:
                    motion_str = "MOVE + TURN"
