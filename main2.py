...
# --- Motion & turning ---
turn = 32                     # max turning PWM output (0..100)
...
accel_rate_turn_up   = 24.0   # ramp when |target| is growing (gentle)
accel_rate_turn_down = 90.0   # ramp when braking / sign change (quick)

alpha = 0.30                  # smoothing factor for person x-position (0-1)
dead_zone_ratio = 0.12        # keep modest; do NOT increase jitter filter via dead-zone
turn_sensitivity = 0.82       # proportional gain (slightly gentler)
turn_D = 0.18                 # derivative gain (more damping than before)
turn_hysteresis = 0.05        # tiny hysteresis band to avoid instant sign flip
zero_cross_band = 0.05        # extra brake band around center
...
SEARCH_SPIN_TURN = 16         # SLOWER spin as requested
        ...
        if detected and not search_mode:
            ...
            if abs(error_ratio) < dead_zone_ratio:
                ...
                if (np.sign(error_ratio) != np.sign(prev_error)) and (abs(error_ratio) < zero_cross_band):
                    ...
                    # PD turn control (damped)
                    dt_small = max(1e-3, now - last_update)
                    d_error = (error_ratio - prev_error) / dt_small
                    d_error = float(np.clip(d_error, -6.0, 6.0))   # clamp derivative
                    prev_error = error_ratio
                    target_turn = np.clip(
                        -turn * (turn_sensitivity * error_ratio + turn_D * d_error),
                        -turn, turn
                    )
